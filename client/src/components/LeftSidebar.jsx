...
const LeftSidebar = ({ authUser, setSidebarOpen }) => {
  ...
  // private chat message + unread reset
  useEffect(() => {
    if (!socket) return;
    ...
    const handleNewMessage = (message) => {
      ...
      dispatch(addMessageInfo({ message, currentUserId }));
      // optional notification
      // notfication.play();
    };

    // unread reset event from backend
  ...
  return (
    <>
      ...
      {/* Secondary Drawer */}
      <aside className={`fixed inset-y-0 left-0 z-40 max-sm:w-full w-lg bg-white dark:bg-[#0e0d0d] border-r border-slate-200 dark:border-white/10 transition-transform duration-300 transform
        md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-18 flex items-center justify-between px-4 border-b border-slate-200 dark:border-white/10">
          <div className="flex items-center gap-2">
            <div className="size-6 bg-indigo-100 rounded flex items-center justify-center text-indigo-600"></div>
            <h1 className="font-semibold dark:text-slate-200 text-gray-900 text-sm">Chatify</h1>
          </div>
          <button onClick={() => dispatch(toggleSidebar())} className="md:hidden p-1">
            <LuX size={20} />
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-7 h-[calc(100vh-120px)]">
          ...
          <div>
            ...
            <div className="px-3 space-y-0.5">
              {chatloading ? <ChatItemSkeleton /> :
                chats.map(chat => {
                  const isActive = selectedUser?._id === chat.user._id;
                  return (
                    <div key={chat.user._id} onClick={() => handleUserClick(chat.user)}
                      className={`flex items-center gap-3 p-3.5 rounded cursor-pointer transition-colors
                      ${isActive ? "bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-white"
                        : "hover:bg-green-900/10 dark:hover:bg-[#343434] text-slate-600 dark:text-white"}`}>
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${chat.user._id}`} alt={chat.user.fullName}
                        className="w-12 h-12 rounded-full bg-gray-200 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between font-spaceGrotesk items-center text-lg font-medium">
                          <span className="truncate">{chat.user.fullName}</span>
                          {chat.lastMessageAt && <span className="text-xs text-green-400 ml-2 whitespace-nowrap">{formatChatTime(chat.lastMessageAt)}</span>}
                        </div>
                        <div className="flex justify-between items-center mt-0.5">
                          <p className="text-xs text-black dark:text-white truncate">{chat.lastMessage || "Start chatting"}</p>
                          {chat.unreadCount > 0 && (
                            <span className="ml-2 bg-green-500 text-white dark:text-black rounded-full min-w-[20px] h-5 px-1 flex items-center justify-center text-xs font-semibold">
                              {chat.unreadCount}
                            </span>
          ...
          <div>
            <div className="px-6 flex items-center justify-between mb-2">
              ...
              <LuCirclePlus size={20} className="cursor-pointer" onClick={() => dispatch(toggleCreateGroupModel())} />
            </div>

            <div className="px-3 space-y-0.5">
              {grouploading ? <ChatItemSkeleton length={2} /> :
                groups.length > 0 ? groups.map(group => {
                  ...
                  return (
                    <div key={group._id} onClick={() => handleGroupClick(group)}
                      ...
                      ${isActive ? "bg-indigo-50 dark:bg-slate-800 text-indigo-700 dark:text-white"
                        : "bg-white dark:bg-[#0e0d0d] hover:bg-green-900/10 dark:hover:bg-[#343434] text-slate-600 dark:text-white"}`}>
                      <img src={group.profilePic || `https://api.dicebear.com/7.x/avataaars/svg?seed=${group._id}`} alt={group.name}
                        className="w-12 h-12 rounded-full bg-gray-200 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center font-spaceGrotesk text-lg font-medium">
                          <span className="truncate">{group.name}</span>
