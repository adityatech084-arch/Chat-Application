...
socket.on("createGroup", async ({ name, memberIds }, callback) => {
  try {
    ...
    // 3️⃣ Emit live update to all connected members
    allMembers.forEach((memberId) => {
...
await User.updateMany(
  ...
  {
    $set: {
      "groups.$[g].lastMessage": text,
      "groups.$[g].lastMessageAt": new Date(),
    },
    $inc: { "groups.$[g].unreadCount": 1 },
  },
  {
  ...
  try {
    ...
    // ------------------- GROUP typing -------------------
    socket.on("group-typing", ({ groupId, groupName }) => {
      io.to(groupId).emit("groupTyping", {
        groupId,
        userId: socket.userId,
        userName: socket.userName, // optional, send from frontend
      });
    });
